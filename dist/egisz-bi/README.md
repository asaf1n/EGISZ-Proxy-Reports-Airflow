...
| Контур | Канал |
| ------ | ----- |
| Airflow | s3-minio, состояние — опрос API |
| DWH | `psql` |
| Metabase | API |

```
egisz-bi/
├─ db/                       # схема DWH, запускается через dwh_init
│  ├─ dwh_init.sql
...
### 3.3. Снимок контура перед импортом (обязательно)

Импорт переписывает карточки целиком: ручные правки, сделанные на контуре (добавленная
колонка, ширины, закрепление колонок), после него не восстанавливаются ниоткуда, кроме
снимка. Снимаются **все** дашборды коллекции, а не только изменяемый:

```bash
METABASE_API_KEY="${METABASE_API_KEY}" MB_URL=https://metabase.example.org python scripts/export_dashboard.py --backup
```

Снимок ложится в `metabase_exports/prod_backup_<дата>_<время>/` и рабочие копии
`metabase_dashboards/` не трогает. Расхождение снимка с репозиторием — это либо правка,
которую нужно перенести в генератор, либо дрейф, который импорт снимет осознанно.
...
### 3.7. Что делает импортёр (идемпотентно, безопасно повторять)
