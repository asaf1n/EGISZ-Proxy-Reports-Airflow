...
# уходит очередь → профиль ожидания → локализация причины → журналы.
SENT_LAYOUT: dict[str, tuple[int, int, int, int]] = {
    # Ряд 1 — состояние: размер очереди, аномалия, последний рабочий срок, крайний возраст.
    QUEUE_SIZE_NAME: (0, 0, 6, 3),
    QUEUE_OVER_24H_NAME: (0, 6, 6, 3),
    QUEUE_RESCUE_NAME: (0, 12, 6, 3),
    QUEUE_MAX_AGE_NAME: (0, 18, 6, 3),
    # Ряд 2 — что происходит с отправленным: слева скорость ответа по всему корпусу,
    # справа баланс очереди за неделю. Один вопрос «успевает ли РЭМД» с двух сторон.
    "Скорость регистрации в РЭМД": (3, 0, 16, 6),
    QUEUE_FLOW_NAME: (3, 16, 8, 6),
    # Ряд 3 — профиль ожидания: срез «сейчас против недели назад» и та же доля по неделям.
    QUEUE_SURVIVAL_NAME: (9, 0, 12, 7),
    QUEUE_TAIL_NAME: (9, 12, 12, 7),
    # Ряд 4 — локализация причины: концентрация по клинике против концентрации по типу.
    QUEUE_PIVOT_CLINIC_NAME: (16, 0, 12, 9),
    QUEUE_PIVOT_SEMD_NAME: (16, 12, 12, 9),
    ...
    "Документы в обработке": (25, 0, 24, 10),
    "Ответ не получен (утилизирован)": (35, 0, 12, 3),
    "Документы: ответ не получен (утилизирован)": (38, 0, 24, 10),
    # Сбой доставки результата в клинику — свой сюжет, внизу вкладки: документ уже
    # получил ответ ЕГИСЗ, ожидания по нему нет.
    "Недоставленные в клинику (ошибка связи шлюз-МО)": (48, 0, 12, 3),
...
CLIENT_SERVICE_LAYOUT: dict[str, tuple[int, int, int, int]] = {
    "Документов за период — клиент": (1, 0, 5, 3),
    "Успешно зарегистрирован": (1, 5, 5, 3),
    "Ошибка асинхронного ответа РЭМД": (1, 10, 5, 3),
    "Ошибка связи": (1, 15, 5, 3),
    "В обработке": (1, 20, 4, 3),
    "Динамика статусов по дням": (4, 0, 24, 7),
    "Топ-10 типов СЭМД по документам": (11, 0, 24, 5),
    "Объёмы ошибок по категориям — клиент": (1, 0, 24, 7),
