...
OPERATIONAL_LAYOUT: dict[str, tuple[int, int, int, int]] = {
    ...
    # Скорость регистрации — про весь корпус, а не про очередь ожидания.
    "Скорость регистрации в РЭМД": (47, 0, 24, 6),
}
...
SENT_LAYOUT: dict[str, tuple[int, int, int, int]] = {
    # Плитка «Отправлено без ответа» снята: после вывода утилизированных из аналитики
    # она совпала с «В обработке». Доля утилизированных считалась от того же корпуса.
    "В обработке": (0, 0, 12, 3),
    "Ответ не получен (утилизирован)": (0, 12, 12, 3),
    # Сводки сверху, построчный разбор под ними: вкладку читают от общей картины
    # к конкретным документам, а не наоборот.
    #
    # Воронка — во всю ширину: восемь шагов в половине сетки режут подписи до «< 12 ча…»,
    # а без читаемой подписи шаг теряет смысл.
    # Горизонтальные срезы — в два столбца. Клиникам шире: подпись содержит JID и
    # наименование, в узкой колонке она обрезается до неразличимого.
    # Воронка очереди на правую границу периода — сводка того же корпуса,
    # что и срезы ниже.
    "В обработке на конец периода": (3, 0, 24, 6),
    "Ожидание ответа по клиникам и ступеням": (9, 0, 14, 9),
    "Ожидание ответа по типам СЭМД и ступеням": (9, 14, 10, 9),
    ...
    "Документы в обработке": (18, 0, 24, 10),
    "Документы: ответ не получен (утилизирован)": (28, 0, 24, 10),
}
...
EXECUTIVE_LAYOUT: dict[str, tuple[int, int, int, int]] = {
    ...
    "Клиники с нулём успехов": (31, 0, 24, 7),
}
