...
RETIRED_CARD_NAMES = frozenset({
    ...
    # Распределение очереди осталось одной карточкой — «Документы в обработке»
    # на оперативном мониторинге; вторая была тем же графиком под другим якорем.
    "Возраст очереди по ступеням",
    # Клиентский дашборд: срез по типам СЭМД остался таблицей «Топ-10 типов СЭМД
    # по документам», она же несёт «% успеха»; доли категорий ошибок вторым рядом
    # вошли в «Объёмы ошибок по категориям — клиент». Обе снятые карточки повторяли
    # запрос соседней.
...
# Лестница ожидания на дашборде называется сроком ожидания: «ступень» и «сегмент» —
# слова модели данных (dim_pending_segments), читателю карточки они ничего не говорят.
WAIT_DIMENSION_LABEL = "Срок ожидания"

QUEUE_SIZE_NAME = f"Документов в очереди{NO_PERIOD_SUFFIX}"
QUEUE_OVER_24H_NAME = f"Ожидают > 24 часов{NO_PERIOD_SUFFIX}"
QUEUE_RESCUE_NAME = f"Ожидают {_queue_rescue_range()}{NO_PERIOD_SUFFIX}"
# Значение упирается в границу последней рабочей ступени: документ старше её уходит из
# очереди в «Ответ не получен». Потолок объявлен в описании карточки, а не в её имени.
QUEUE_MAX_AGE_NAME = f"Самый давний в очереди, суток{NO_PERIOD_SUFFIX}"
# Распределение очереди по срокам — одна карточка на оперативном мониторинге.
# Срез профиля ожидания разбирается кривой долей и матрицами на вкладке «Отправленные».
QUEUE_NOW_NAME = f"Документы в обработке{NO_PERIOD_SUFFIX}"
QUEUE_SURVIVAL_NAME = f"Время ожидания ответа: сейчас и неделю назад{NO_PERIOD_SUFFIX}"
QUEUE_PIVOT_CLINIC_NAME = f"Клиника × срок ожидания{NO_PERIOD_SUFFIX}"
QUEUE_PIVOT_SEMD_NAME = f"Тип СЭМД × срок ожидания{NO_PERIOD_SUFFIX}"
QUEUE_FLOW_DAYS = 7
...
# Слова модели данных («ступень», «хвост») с лица дашборда сняты: лестница ожидания
# называется сроком ожидания, а карточка — тем, что на ней написано. Источников у прежнего
# имени два — репозиторный JSON и правки, сделанные прямо на контуре, — поэтому в карте
# стоят оба варианта.
RENAME_01 = {
    f"Ждут {_queue_rescue_range()}{NO_PERIOD_SUFFIX}": QUEUE_RESCUE_NAME,
    f"Ожидают  {_queue_rescue_range()}{NO_PERIOD_SUFFIX}": QUEUE_RESCUE_NAME,
    f"Максимальный возраст в очереди{NO_PERIOD_SUFFIX}": QUEUE_MAX_AGE_NAME,
    f"Максимальный возраст в очереди 15=max{NO_PERIOD_SUFFIX}": QUEUE_MAX_AGE_NAME,
    f"Текущая очередь по ступеням{NO_PERIOD_SUFFIX}": QUEUE_NOW_NAME,
    f"Доля ожидающих дольше срока{NO_PERIOD_SUFFIX}": QUEUE_SURVIVAL_NAME,
    f"Динамика времени регистрации документов{NO_PERIOD_SUFFIX}": QUEUE_SURVIVAL_NAME,
    f"Клиника × ступень{NO_PERIOD_SUFFIX}": QUEUE_PIVOT_CLINIC_NAME,
...
RENAME_OTHER: dict[str, dict[str, str]] = {
    ...
    "05_executive.json": {
        "Зависших >7 дн., %": "Ответ не получен, %",
        # Метрика не была first-pass: пересдачи в ней не различались, а знаменателем
...
# Тот же грейн и та же метка состояния, что в STATUS_BY_DAY_QUERY, — доли считает
# визуализация (нормированный стэк), а не запрос. «Без ответа» исключено: ответа по ним
# уже не ожидается, разбор — на вкладке «Отправленные».
...
# Подпись столбца — полное наименование ЮЛ: под наклонной осью оно занимает половину
# высоты карточки и вытесняет сам график, поэтому обрезается до читаемой длины.
CLINIC_ERROR_VOLUME_LABEL_CHARS = 35
...
SENT_FILTER_TEMPLATE_TAGS = {
    ...
    "pending_segment": {
        "widget-type": "string/=",
        "display-name": WAIT_DIMENSION_LABEL,
...
# Правило цвета на дашбордах: красный-зелёный несёт оценку «хорошо/плохо» (доли успеха и
# ошибок), бело-синий — только величину. Счётчики документов оценки не несут: очередь в
# 200 документов не «хуже» очереди в 20, если сроки те же, — поэтому счётные карточки и
# тепловые карты по счётчику красятся одним синим градиентом.
QUEUE_COUNT_COLOR = STATUS_DETAIL_COLORS["В обработке"]["color"]
COUNT_HEATMAP_COLORS = ["#FFFFFF", "#2D7DD2"]
# Шкала оценки: минимум — хорошо, максимум — плохо. Порядок цветов задаёт направление,
# перевёрнутый набор красит нулевую долю отказов красным.
SHARE_QUALITY_COLORS = ["#84BB4C", "#FBBF24", "#DC2626"]
...
QUEUE_NOW_QUERY = (
    SENT_QUEUE_CORPUS
    + f' SELECT segment_label AS "{WAIT_DIMENSION_LABEL}", '
    'COUNT(DISTINCT dwh_id)::bigint AS "Документов" '
...
# Доля долгого ожидания по неделям: очередь на конец каждой недели и её части старше
# порогов справочника. Считается тем же предикатом на недельных якорях — недельная витрина
# разреза по ступеням не несёт, а фильтры вкладки должны действовать полностью.
QUEUE_TAIL_WEEKS = 12

# Членство в очереди развёрнуто предикатом вместо вызова is_pending_at и
# pending_segment_code_at на каждую пару «неделя × документ»: рабочая ступень — это
# возраст не старше последнего конечного порога справочника, поэтому набор недели
...
QUEUE_TAIL_QUERY = (
    ...
    "ROUND(100.0 * COUNT(DISTINCT q.dwh_id) FILTER (WHERE q.age_minutes > t.m_72h) "
    '/ NULLIF(COUNT(DISTINCT q.dwh_id), 0), 1) AS "> 3 суток, %", '
...
SENT_TAB_CARD_PLAN: tuple[tuple[str, str, str], ...] = (
    ...
    (QUEUE_NOW_NAME, "row", "operational"),
)
...
#
# Срок считается до ПЕРВОГО ответа — тем же событием, которым документ выходит из очереди
# (README §«Учёт отправленных»). delivery_seconds для воронки не годится: он меряет срок до
# последнего повторного ответа, и документ, отвеченный за секунды, попадал на нижние шаги
# по возрасту своего повтора. Расхождение измеримое: на границе «< 5 минут» это 3,6 тыс.
# документов, а 834 документа не попадали и на самый мягкий шаг «< 15 суток» — при том что
# по правилу очереди столько не ждал никто.
SENT_REGISTRATION_FUNNEL_QUERY = (
    ...
    # first_callback_at IS NOT NULL: срока у них нет, и в знаменателе они притворялись бы
    # медленными, хотя первые ещё в очереди, а вторым ответа не будет вовсе. Ответ раньше
    ...
    "UNION ALL "
    # Метка срока из справочника («до 6 часов») сжимается до знака сравнения: восемь
...
SENT_TAB_DESCRIPTIONS: dict[str, str] = {
    SENT_STATE_NO_RESPONSE_LABEL: (
        "Сколько отправленных документов перешагнуло 15 суток без ответа: ответа по ним "
        "уже не будет. Реквизиты: количество документов. Из карточек очереди такие "
        "документы исключены; список — в таблице ниже."
    ),
    ...
    SENT_REGISTRATION_FUNNEL_NAME: (
        ...
        "шаг — весь такой корпус за период, дальше — сколько документов уложилось в срок; "
        "сроки ужесточаются слева направо, поэтому воронка сужается. Срок считается от "
        "запроса файла до первого ответа ЕГИСЗ. Реквизиты: этап (срок), количество "
        "документов, доля от корпуса. Ожидающие и «Ответ не получен» не входят: срока "
        "регистрации у них ещё нет."
    ),
    QUEUE_SIZE_NAME: (
    ...
    QUEUE_RESCUE_NAME: (
        ...
        "смысл разбираться: после 15 суток ответ не приходит. Реквизиты: количество "
        f"документов. Целевое значение — ноль. {_QUEUE_SCOPE}"
    ),
    QUEUE_MAX_AGE_NAME: (
        "Сколько суток ждёт самый давний документ очереди. Возраст считается от запроса "
        "файла ЕГИСЗ. Реквизиты: срок ожидания в сутках. Значение упирается в 15 суток — "
        "дальше документ уходит в «Ответ не получен» и в очереди не числится, поэтому "
        f"15,0 означает «на верхнем сроке есть документы». {_QUEUE_SCOPE}"
    ),
    QUEUE_NOW_NAME: (
        "Из чего складывается очередь прямо сейчас: строка — срок ожидания, длина полосы — "
        "сколько документов ждёт ровно столько. Реквизиты: срок ожидания, количество "
        "документов. Сроки взаимоисключающие, сумма полос равна размеру очереди. "
        f"{_QUEUE_SCOPE}"
    ),
    QUEUE_SURVIVAL_NAME: (
        "Насколько затянулось ожидание: точка — доля очереди, которая ждёт дольше "
        "указанного срока. Реквизиты: срок ожидания, срез (сейчас / неделю назад), доля "
        "очереди в процентах. Ряд накопительный — в отличие от взаимоисключающих сроков "
        "карточки «Документы в обработке» на вкладке «Оперативный мониторинг». Второй ряд "
        "показывает ту же картину неделю назад: сравнивается форма ожидания, а не размер "
        "очереди. Крайняя правая точка всегда нулевая: документ старше 15 суток из очереди "
        f"уже вышел. {_QUEUE_SCOPE}"
    ),
    QUEUE_PIVOT_CLINIC_NAME: (
        "Где стоит очередь: строка — клиника, колонка — срок ожидания, в ячейке — "
        "количество документов. Реквизиты: клиника, сроки ожидания, итог по строке. "
        "Заливка ячейки показывает величину, а не оценку. Скопление на дальних сроках у "
        "одной клиники указывает на транспорт с её стороны; ровное распределение по многим "
        f"клиникам разбирается соседней таблицей по типам СЭМД. {_QUEUE_SCOPE}"
    ),
    QUEUE_PIVOT_SEMD_NAME: (
        "Что стоит в очереди: строка — код СЭМД, колонка — срок ожидания, в ячейке — "
        "количество документов. Реквизиты: код СЭМД, сроки ожидания, итог по строке. "
        "Скопление по одному коду при ровном распределении по клиникам указывает на "
        f"обработку этого типа документа на стороне ЕГИСЗ. {_QUEUE_SCOPE}"
    ),
    QUEUE_FLOW_NAME: (
        f"Баланс очереди за {QUEUE_FLOW_DAYS} суток: что стояло на начало окна, что "
        "отправлено внутри него, что ушло полученным ответом и что списано по сроку. "
        "Реквизиты: этап, количество документов, итог. Сумма шагов равна очереди на конец "
        "окна. Оба оттока окрашены одинаково: у водопада один цвет убыли, шаги различаются "
        f"подписью. {_QUEUE_SCOPE}"
    ),
    QUEUE_TAIL_NAME: (
        "Тянется ли ожидание из недели в неделю: точка — доля очереди на конец недели, "
        "которая ждала дольше 24 часов, 3 и 7 суток. Реквизиты: неделя (МСК), три доли в "
        "процентах. Последняя точка — на текущий момент. Недели левее начала накопления "
        "истории пустые: журнал обмена выгружается на ограниченную глубину, и очередь на "
        f"более ранний момент восстанавливать не из чего. {_QUEUE_SCOPE}"
    ),
}
...
def apply_clinic_error_volume(card: dict) -> None:
    ...
    # Наименования клиник под наклоном и подписи значений в сжатом виде: карточка занимает
    # чуть больше половины ширины сетки, горизонтальные подписи в ней не помещаются.
    viz["graph.x_axis.axis_enabled"] = "rotate-45"
    viz["graph.x_axis.scale"] = "ordinal"
    viz["graph.label_value_formatting"] = "compact"
    viz.pop("graph.x_axis.labels_enabled", None)
...
def apply_top_semd_by_errors(card: dict) -> None:
    ...
    # «%» — доля отказов внутри типа СЭМД: чем выше, тем хуже. Шкала шла в обратную
    # сторону — нулевой процент красился красным, а половина отказов зелёным.
    viz["table.column_formatting"] = [
        {
            "columns": ["%"],
            "type": "range",
            "colors": list(SHARE_QUALITY_COLORS),
            "min_type": "custom",
            "max_type": "custom",
            "min_value": 0,
            "max_value": 50,
...
def apply_sent_card_visualization(card: dict, name: str, viz: dict) -> None:
    ...
    if name == SENT_REGISTRATION_FUNNEL_NAME:
        ...
        # Горизонтальные полосы: подписи сроков читаются строкой и не режутся, как на
        # вертикальной оси. Ряд один — счётчик документов, поэтому и цвет один: лестница
        # оттенков здесь дублировала бы подпись строки, а не несла своё значение.
        card["display"] = "row"
        viz["graph.dimensions"] = [WAIT_DIMENSION_LABEL]
        viz["graph.metrics"] = ["Документов"]
        viz["graph.x_axis.scale"] = "ordinal"
        viz["graph.x_axis.title_text"] = ""
        viz["graph.x_axis.labels_enabled"] = False
        viz["graph.y_axis.title_text"] = "Документов"
        viz["graph.show_values"] = True
        viz["series_settings"] = {"Документов": {"color": QUEUE_COUNT_COLOR}}
        viz["column_settings"] = {
            _column_key("Документов"): {"decimals": 0, "number_separators": ", "}
        }
        ...
        # условным форматированием, общий для всех колонок — иначе они несравнимы.
        # В ячейке стоит счётчик документов, а не оценка, поэтому градиент бело-синий:
        # красный здесь читался бы как «плохо» на любом заполненном сроке ожидания.
        viz["table.column_formatting"] = [
            {
                "type": "range",
                "columns": segments,
                "colors": list(COUNT_HEATMAP_COLORS),
                "min_type": "all",
                "max_type": "all",
            }
        ]
...
# Вкладка собирается целиком этим блоком: состав карточек, знаменатели, подписи и сетка
# объявлены в одном месте, поэтому точечная правка одной плитки не расходится с соседней.
EXECUTIVE_DASHBOARD_FILE = "05_executive.json"
EXECUTIVE_TAB = "overview"
...
EXECUTIVE_PENDING = "status = 'sent' AND sent_state <> 'no_response'"

# Абонплата за JID. Договорная сетка сложнее плоской ставки, поэтому рублёвые карточки —
...
def _executive_money(name: str, *, decimals: int = 0) -> dict:
    return {
        "scalar.field": name,
        "column_settings": {
            f'["name","{name}"]': {
...
def _executive_tag(kind: str, seq: int) -> dict:
    prefix = "05e10000" if kind == "ips_date" else "05e30000"
    return {
        "widget-type": "date/all-options" if kind == "ips_date" else "string/=",
        "display-name": "Период" if kind == "ips_date" else "Клиника",
        "id": f"{prefix}-0000-4000-8000-{seq:012x}",
        "name": kind,
        "type": "dimension",
    }
...
def _executive_card(
    ...
    return {
        ...
        "dataset_query": {
            ...
            "native": {
                "query": query,
                "template-tags": {kind: _executive_tag(kind, seq) for kind in kinds},
            },
            "database": 1,
        },
        "display": display,
...
def _executive_text(text: str, layout: tuple[int, int, int, int]) -> dict:
    row, col, size_x, size_y = layout
    return {
        "text": text,
        "display": "text",
...
def _executive_active_base(tail: str) -> str:
...
def _executive_daily_base(metric: str, alias: str) -> str:
    ...
    return (
        ...
        "'1 day'::interval)::date AS day) "
        f'SELECT d.day AS "Дата", {metric} AS "{alias}" '
        "FROM days d LEFT JOIN jid_activity ja ON ja.activity_day >= d.day - "
        "INTERVAL '30 days' AND ja.activity_day <= d.day GROUP BY d.day ORDER BY d.day"
    )


def _executive_daily_viz(alias: str, color: str, suffix: str | None) -> dict:
    value_format: dict = {"decimals": 0, "number_separators": ", "}
    if suffix:
        value_format["suffix"] = suffix
    return {
        ...
        "column_settings": {
            f'["name","{alias}"]': value_format,
            '["name","Дата"]': {"column_title": "Дата"},
        },
    }


def _executive_per_jid(*aggregates: str) -> str:
    return (
...
_EXECUTIVE_DOCS = "COUNT(DISTINCT dwh_id) AS docs"
...
def executive_overview_cards() -> list[dict]:
    ...
    return [
        ...
        _executive_card(
            ...
            "COUNT(DISTINCT документ) по всем состояниям, включая «в обработке» и «ответ не "
            "получен». Общий объём потока; знаменателем долей качества не служит.",
        ...
        _executive_card(
            ...
            _executive_active_base(
                'SELECT COUNT(*)::bigint AS "Активных JID" FROM active_jid'
            ),
        ...
        _executive_card(
            ...
            _executive_active_base(
                ...
                '/ NULLIF(success.ok, 0), 1) AS "₽ за успешный СЭМД" FROM success'
            ),
        ...
        _executive_card(
            ...
            + "SELECT CASE WHEN docs >= 1000 THEN 'Тяжёлые (от 1000 док.)' "
            "WHEN docs >= 50 THEN 'Средние (50–999)' ELSE 'Спящие (менее 50)' END "
            'AS "Сегмент", COUNT(DISTINCT jid)::bigint AS "Клиник", SUM(docs)::bigint AS "Документов", '
            'SUM(ok)::bigint AS "Успешных СЭМД", '
            'ROUND(100.0 * SUM(ok) / NULLIF(SUM(answered), 0), 1) AS "Доля успеха, %", '
            f'(COUNT(DISTINCT jid) * {tariff})::bigint AS "MRR, ₽", '
            f'ROUND(COUNT(DISTINCT jid) * {tariff}.0 / NULLIF(SUM(ok), 0), 0) AS "₽ за успешный СЭМД" '
            "FROM per_jid GROUP BY 1 "
...
def apply_client_status_by_day(card: dict) -> None:
    """Состояния документа за день на клиентском дашборде.

    Грейн и метка состояния — те же, что у «Статусов регистрации СЭМД» на дашборде
    интеграции; клиентский вид отличается только нормировкой стэка и набором фильтров.
    """
    ...
    for key in list(viz.keys()):
        if (
            ...
            or key in ("series_settings", "stackable.stack_type", "column_settings")
        ):
            del viz[key]
    viz["graph.dimensions"] = ["Дата", "Статус"]
    viz["graph.metrics"] = ["Документов"]
    ...
    viz["graph.y_axis.title_text"] = "% документов"
    viz["graph.show_values"] = False
    viz["stackable.stack_type"] = "normalized"
    viz["series_settings"] = {
        k: deepcopy(v) for k, v in STATUS_DETAIL_COLORS.items() if k != "Без ответа"
    }
    viz["column_settings"] = {
...
def apply_client_error_category_volume(card: dict) -> None:
    """Объём и доля по категориям ошибок в одной карточке.

    Счёт и доля живут на разных осях — тот же приём, что на «Ошибки по клиникам:
    объём и %» дашборда интеграции.
    """
    card["display"] = "combo"
    viz = card.setdefault("visualization_settings", {})
    viz.pop("pie.rows", None)
    viz["graph.dimensions"] = ["Категория ошибки"]
    viz["graph.metrics"] = ["Документов", "% ошибок"]
    viz["graph.x_axis.axis_enabled"] = "rotate-45"
    viz["graph.x_axis.scale"] = "ordinal"
    viz["graph.x_axis.title_text"] = ""
    ...
    viz["graph.label_value_formatting"] = "compact"
    viz["series_settings"] = {
        "Документов": {"axis": "left", "color": "#DC2626", "display": "bar"},
        "% ошибок": {
            "axis": "right",
            "color": "#F2994A",
            "display": "line",
    ...
    viz["column_settings"] = {
        '["name","Документов"]': {"decimals": 0, "number_separators": ", "},
        '["name","% ошибок"]': {"decimals": 1, "number_separators": ", ", "suffix": " %"},
    }


def apply_client_error_dynamics(card: dict) -> None:
    """Категории ошибок красятся общей палитрой — цвет категории один на всех отчётах."""
    viz = card.setdefault("visualization_settings", {})
    series = viz.setdefault("series_settings", {})
    series.update({cat: {"color": color} for cat, color in CATEGORY_COLORS.items()})


CLIENT_CARD_APPLIERS = {
    "Динамика статусов по дням": apply_client_status_by_day,
    "Объёмы ошибок по категориям — клиент": apply_client_error_category_volume,
    "Динамика ошибок по дням — клиент": apply_client_error_dynamics,
}
...
def apply_client_dashboards(path: Path) -> bool:
    ...
    for card in dash.get("cards", []):
        ...
        applier = CLIENT_CARD_APPLIERS.get(card.get("name"))
        if applier and card.get("dataset_query", {}).get("type") == "native":
            applier(card)
        dq = card.get("dataset_query", {})
        if dq.get("type") == "native":
