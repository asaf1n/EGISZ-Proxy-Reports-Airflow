...
AS $$
    SELECT CASE
        ...
        WHEN COALESCE(p_msgtext, '') ~* '<(ns[0-9]+:)?(error|fault)|<faultstring|<errorCode'  THEN 'error'
        -- Асинхронное подтверждение регистрации в РЭМД (финальный success).
        WHEN public.egisz_xml_text(p_msgtext, 'documentStatus') ~* 'зарегистр'               THEN 'success'
        WHEN COALESCE(p_msgtext, '') ~* 'registerDocumentResult'
        ...
        WHEN COALESCE(p_msgtext, '') ~* 'RegisterDocumentResponse'
