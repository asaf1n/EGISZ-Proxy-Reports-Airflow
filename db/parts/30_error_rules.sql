...
VALUES
    ...
    -- Расхождение ФИО пациента: конкретные значения в скобках → нормализуем в единый тип
    ('patient_fio_mismatch', 72, NULL, '(?is)(Имя|Фамилия|Отчество) пациента в ЭМД \[.*?\] отличается', 'ФИО пациента в ЭМД не соответствует данным ЕГИСЗ', 'Данные пациента'),
    -- Расхождение пола пациента: числовые значения 0/1/2 в скобках → нормализуем
    ('patient_gender_mismatch', 72, NULL, '(?is)Пол пациента в ЭМД \[.*?\] отличается', 'Пол пациента в ЭМД не соответствует данным ЕГИСЗ', 'Данные пациента'),
    -- Кросс-валидация RegisterHealthDocument: РЭМД сверяет общие сведения документа в
    -- запросе на регистрацию с содержимым СЭМД. Конкретные значения (id ЭМД/запроса, дата,
    -- структурное подразделение) приходят в [квадратных скобках] и делают каждое сообщение
    -- уникальным, раздувая кардинальность error_type. Нормализуем в канонический тип.
    ('document_uid_mismatch_request', 48, NULL, '(?is)Уникальный идентификатор документа в ЭМД \[.*?\] отличается', 'Идентификатор документа в ЭМД не совпадает с идентификатором в запросе на регистрацию', 'Ошибки регистрации в РЭМД'),
    ('document_creation_date_mismatch_request', 48, NULL, '(?is)Дата создания документа в ЭМД \[.*?\] отличается', 'Дата создания документа в ЭМД не совпадает с датой в запросе на регистрацию', 'Ошибки регистрации в РЭМД'),
    ('signature_mo_date_after_request', 49, NULL, '(?is)Дата и время создания подписи МО \[.*?\] не может быть позже', 'Дата подписи МО позже даты поступления запроса на регистрацию', 'Ошибки регистрации в РЭМД'),
    ('provider_org_mismatch_request', 49, NULL, '(?is)не совпадает с СП providerOrganization', 'Структурное подразделение (providerOrganization) в СЭМД не совпадает с запросом на регистрацию', 'Ошибки регистрации в РЭМД'),
    ('represented_org_mismatch_request', 49, NULL, '(?is)не совпадает с СП representedOrganization', 'Структурное подразделение (representedOrganization) в СЭМД не совпадает с запросом на регистрацию', 'Ошибки регистрации в РЭМД'),
    ('emd_version_registration_forbidden', 49, NULL, '(?is)запрещена регистрация новых версий', 'Для данного вида ЭМД запрещена регистрация новых версий', 'Ошибки регистрации в РЭМД')
