...
-- Тип ошибки — формулировка РЭМД. Интерпретация правила показывается только там, где
-- текста нет или он технический (XPath, XML, исключения); для категории она остаётся
-- ключом classification_type. Явные диапазоны кириллицы работают при lc_ctype=C.
...
-- Формулировка без значений экземпляра: тип уходит в фильтры и сводки дашбордов, в том
-- числе клиентские. Хвост с реквизитами сертификата (субъект, серийный номер, e-mail)
                        ...
                        regexp_replace(
                            regexp_replace(
                                regexp_replace(
                                    regexp_replace(
                                        regexp_replace(
                                            regexp_replace(
                                                ...
                                                --
                                                -- Значения в кавычках и адреса e-mail принадлежат
                                                -- экземпляру (фамилии пациентов, почта сотрудников):
                                                -- тип без правила уходит в фильтры дашбордов как есть.
                                                regexp_replace(
                                                    regexp_replace(
                                    ...
                                    --
                                    -- Граница слова и регистронезависимость для кириллицы зависят
                                    -- от locale базы: под lc_ctype = C (так на внешнем контуре)
                                    -- ни `\y` перед «У», ни `(?i)` для неё не срабатывают, и правило
                                    -- молча становится мёртвым. Поэтому граница задана явным классом
                                    -- (совпавший символ возвращается группой \1), а регистр — набором:
                                    -- диапазоны сравниваются по коду символа и от locale не зависят.
                                    '(^|[^0-9A-Za-zА-Яа-яЁё])[Уу]\d+(?:[-.]\d+)+',
                                    '\1<правило>',
                                    'g'
                                ),
                                '\y\d+(?:\.\d+){3,}\y',
