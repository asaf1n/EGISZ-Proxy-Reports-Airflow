...
| `extract_and_load_batch` | Read watermark from `elt_state` (`last_logid` + `source_min_created_at`), pull the EXCHANGELOG batch by `LOGID > last_logid` bounded by `source_min_created_at`, and UPSERT it into `exchangelog_raw` **in-process** (extract + load fused). Returns XCom `{count, last_logid, cursor_logid}` |
...
| `reconcile_late_arrivals` | **Self-healing safety net.** The proxy journal materializes rows **out of LOGID order** — async СЭМД callbacks and gateway backfills land below an already-advanced watermark, so the forward `LOGID > last_logid` cursor loses them permanently. Each cycle this task diffs the proxy's date-eligible LOGID set against `exchangelog_raw` and load+transform whatever is missing **below** the watermark, **without moving the watermark** (GREATEST stays the only writer of `last_logid`). No-op in steady state. **Do not remove** — see §8. |
...
# input and returns an int (rows reconciled) — it reads its own state from elt_state.
```
...
| Remove `reconcile_late_arrivals` or treat `LOGID > last_logid` as complete | The proxy journal materializes rows **out of LOGID order** (late СЭМД callbacks / gateway backfills land below the watermark). The forward cursor alone loses them permanently — `reconcile_late_arrivals` set-diffs proxy↔`exchangelog_raw` each cycle and recovers them |
| Recover stranded-below-watermark rows by rolling `last_logid` back | Let `reconcile_late_arrivals` load+transform the diff **without** moving the watermark; if recovering manually, load+`egisz_transform_raw_to_facts(min_missing-1, max_missing)` for the missing set, never lower `last_logid` |
