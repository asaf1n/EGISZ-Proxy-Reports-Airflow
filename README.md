...
Для каждого элемента ответа сохраняется пара `code` / `message`. Тип ошибки — формулировка ответа РЭМД без значений экземпляра (`error_message_type`): значения в скобках, кавычках, даты, ФИО со СНИЛС и e-mail заменяются на `[…]`, хвост с реквизитами сертификата отрезается; наименование реквизита в «Указанное значение [Имя пациента] …» сохраняется. Интерпретация правила выводится только для технического текста (XPath, XML, исключения) и ответа без текста; для категории, кода и зоны ответственности она служит ключом `classification_type`. Полный текст хранится в `message` и `error_text`.
