...
```text
Firebird proxy_egisz (EXCHANGELOG, справочники)
    → exchangelog_raw (raw staging)
    → dim_egisz_exchangelog_refs (разложение XML, 1 проход на LOGID)
    → fact_egisz_documents, fact_egisz_transactions
    → v_egisz_documents_enriched_ui, v_egisz_documents_daily_ui
    → v_rpt_*_ui, v_health_*_ui
    → Metabase (дашборды и SQL-карточки)
```

Оркестрация — DAG `egisz_elt_dag`, расписание `*/5 * * * *`: инкрементальная загрузка по курсору `LOGID`, трансформация в PostgreSQL, сопровождение витрин, продвижение watermark, дозагрузка опоздавших строк журнала. Отчётность читает persistent-слой (facts, dimensions, витрины), а не raw staging.

Единый источник описания проекта — этот файл (`README.md`): предметная область, архитектура, DWH, парсинг, дашборды и эксплуатация. При разработке и автоматизации ориентироваться на README и общие инструкции репозитория; доменные детали в код и комментарии не дублировать — ссылаться на соответствующий раздел README.
...
Правила подсчёта метрик, статусы документов и grain отчётных витрин — в разделе [Правила аналитики](#правила-аналитики). Дашборды, healthcheck и финансовая модель — в разделе [Дашборды Metabase](#дашборды-metabase).
...
|---|---|---|
| Извлечение | Python (Airflow tasks) | Keyset-чтение Firebird, UPSERT в `exchangelog_raw`, синхронизация справочников, watermark, дозагрузка опоздавших `LOGID`. |
| Трансформация | PL/pgSQL | Парсинг SOAP/XML, классификация статусов и ошибок, сборка facts, инкрементальное сопровождение `v_egisz_documents_enriched_ui`. |
| Публикация | Metabase | SQL-карточки поверх `v_rpt_*_ui` и `v_health_*_ui`; field-фильтры по `field_filter_defaults.yaml`. |

Курсор загрузки — `elt_state.last_logid` (продвижение через `GREATEST`). Нижней отсечки по дате нет: выборка идёт по `LOGID`, а не по календарному окну.
...
| `reconcile_late_arrivals` | Дозагрузка опоздавших строк журнала. Сравнивает полосу `LOGID` прокси непосредственно под watermark с `exchangelog_raw`, догружает и трансформирует недостающее, не изменяя `last_logid`. При отсутствии пропусков — no-op. |

#### Дозагрузка опоздавших строк

Прокси-журнал материализует строки не строго по возрастанию `LOGID`: асинхронные callback'и СЭМД и backfill'ы шлюза могут появляться с `LOGID` ниже уже продвинутого watermark. Forward-выборка `LOGID > last_logid` такие строки не охватывает.

Задача `reconcile_late_arrivals` на каждом цикле сравнивает множество `LOGID` прокси в полосе `(last_logid - RECONCILE_WATERMARK_LOOKBACK_LOGIDS, last_logid]` с `exchangelog_raw` и догружает недостающее. Курсор `last_logid` не сдвигается назад; единственный писатель курсора — `update_watermark` через `GREATEST(current, new)`. Восстановленные `LOGID` трансформируются плотными окнами, а не одним диапазоном `min..max`.

Полоса задаётся по `LOGID`, без фильтра по дате. Параметр `RECONCILE_WATERMARK_LOOKBACK_LOGIDS` в DAG (по умолчанию `200000`) определяет ширину полосы: она должна перекрывать типичный разброс внеочередного наполнения журнала (порядка десятков тысяч `LOGID` ниже watermark). Увеличение полосы расширяет охват старых строк и увеличивает объём сканирования. Связь восстановленного callback'а с документом выполняется в transform по `relates_to_id` → `dim_egisz_exchangelog_refs.exchange_msgid_norm`, без повторного сканирования `MSGTEXT`.
...
| Витрины | `v_egisz_documents_enriched_ui` (persistent-таблица + источник `v_egisz_documents_enriched_src`), `v_egisz_documents_daily_ui` (materialized view) | Витрины Metabase поверх document-grain facts. `enriched_ui` сопровождается инкрементально в `egisz_transform_raw_to_facts` по затронутым `document_key`, без полного REFRESH; дневной rollup `daily_ui` — materialized view с полным REFRESH в `refresh_materialized_views`. |
...
Единица учёта документа — `document_key` = `lower(localUid)`. Все агрегации «по документам» в DWH и Metabase используют `COUNT(DISTINCT «Документ (ключ учёта)»)`, если в карточке не указано иное.

| Правило | Формула / условие | Где применяется |
|---|---|---|
| Универсум документов | `COUNT(DISTINCT «Документ (ключ учёта)»)` по всем 4 статусам | KPI «всего документов», статусные диаграммы (сумма сегментов = итог, включая «В обработке») |
| Доля успеха / ошибок (%) | Знаменатель: финализированные документы (`success` + `async_error` + `network_error`); «В обработке» исключается | Дашборды 01, 02, 04 |
| Универсум ошибок | `async_error` + `network_error`; одинаковый во всех срезах по ошибкам | Категории и типы ошибок, рейтинги клиник/СЭМД |
| First-pass acceptance (%) | `success` / все документы, включая `waiting` | Дашборд 05 |
| Очередь ожидания | `status = waiting`; категория «просрочено» — ожидание > 72 ч | Дашборд 03 (`v_rpt_documents_no_response_ui`) |
| Активный JID | `DISTINCT JID`, у которого был ≥ 1 документ за trailing-30d | Финансовые карточки 05 (MRR, ARR) |
| Grain отчётной строки | Одна актуальная строка на `document_key` в `v_rpt_documents_ui` (дедупликация по LOGID и статусу) | Все документные дашборды 01–06 |
...
Колонка «Статус» во всех таблицах и графиках содержит адаптированный русский текст (4 значения в таблице выше); машинный код для фильтров и агрегаций — «Статус (код)». Статус пересчитывается при каждом обновлении документа в `egisz_transform_raw_to_facts`; последний callback определяется через `GREATEST(last_callback_at)`.
...
Часть сообщений РЭМД встраивает в текст конкретные значения в `[квадратных скобках]` (id ЭМД и запроса, дата/время, структурное подразделение). Без нормализации каждое такое сообщение формирует отдельный `error_type` и увеличивает число distinct-значений. Правила кросс-валидации `RegisterHealthDocument` сворачивают эти семьи в канонический тип по текстовому паттерну (`\[.*?\]` поглощает изменчивое значение); аналогичная свёртка применена для ФИО и пола пациента.
...
JSON-описания дашбордов — в `metabase_dashboards/`. Импорт: `metabase/setup-dashboards.sh` при старте контейнера и через `kubectl exec` в `up.ps1 -Action Metabase`. Field-фильтры настраивает `scripts/apply_metabase_field_filters.py` по правилам `metabase_dashboards/field_filter_defaults.yaml`.

Состав: 8 дашбордов, 106 SQL-карточек. Источники — только persistent-слой (`v_rpt_*_ui`, `v_health_*_ui`), без чтения `exchangelog_raw`.

### Период и фильтры

У дашбордов 01–06 период по умолчанию — `past30days~`; у клиентских 07/08 — `past7days~` / `past30days~` и примерный `JID` для локального стенда. Суффикс `~` означает «включая текущий день»: без него относительный фильтр Metabase обрезает окно по концу прошлых суток, и документы за сегодня не попадают в выборку. Суффикс также предотвращает ситуацию с пустыми query-параметрами (`?dwh_date_filter=`) в URL, при которой Metabase с `auto_apply_filters` не отрисовывает привязанные карточки. Переменная `METABASE_AUTO_APPLY_FILTERS` (по умолчанию `true`) задаётся в `setup-dashboards.sh`. Проверка импорта и SQL-карточек: `py scripts/audit_metabase_dashboards.py` (нужен Metabase на `localhost:3000`).

Привязка фильтров. Фильтр дашборда привязывается к карточке, если витрина-источник содержит соответствующую колонку (dimension field filter из `field_filter_defaults.yaml`). Если колонки нет, фильтр к карточке не привязывается — карточка сохраняет собственный охват данных. Пример: `v_rpt_error_category_breakdown_ui` не содержит «Статус» и идентификаторов документа, поэтому карточки «Ошибки по типу/виду» реагируют на период, код СЭМД и JID, но не на статус и `localUid`. Снимки текущего состояния (healthcheck, сводка прокси-БД, окна 24ч/72ч) и метрики trailing-30d (MRR, ARR) от фильтра «Период» не зависят; это отражено в названии карточки. На дашбордах 01, 02, 06 идентификаторы (`localUid`, `relatesToMessage`, `emdrId`, `LOGID`) и статус протянуты во все документные карточки для согласованного точечного поиска.

Ось времени графиков совпадает с фильтром периода, если в карточке не указано иное. Почасовые тренды на `02` используют `dwh_date_filter` по «Обработано IPS». На `04` блок транспортных ошибок использует отдельный `parse_created_filter` по «Дата создания документа».

Палитра статусов единая во всех диаграммах: Успешно зарегистрирован — `#84BB4C`, Ошибка асинхронного ответа РЭМД — `#A989C5`, Ошибка связи — `#F2994A`, В обработке — `#509EE3`. Цвета заданы в `visualization_settings` карточек (`pie.rows` / `series_settings`).

### Healthcheck и инциденты

Дашборд `02_service.json` — основной контур мониторинга ELT и транспортных сбоев. Источники сигналов — view `v_health_signals_ui`, `v_health_by_clinic_ui`, `v_health_proxy_db_ui` (определены в `db/parts/90_views_health_and_finalize.sql`).

| Сигнал (`v_health_signals_ui`) | Условие расчёта | Уровень | Действие |
|---|---|---|---|
| `parsed_documents` | `COUNT(*)` из `fact_egisz_documents` | green | Контроль поступления СЭМД в DWH |
| `queue_24h` | `waiting` и `sent_at` < now() − 24 ч | yellow | Проверить клиники с зависшими документами и транспортный канал |
| `network_errors` | `status = network_error` | yellow | Разбор топов формулировок и последних событий на дашборде 02 |
| `error_rows` | `status = async_error` | yellow | Разбор отказов РЭМД на дашбордах 04 и 05 |
| `pending_backlog_24h` | `waiting` и `sent_at` < now() − 72 ч | green / yellow / red (≥ 20 / ≥ 100) | Детализация очереди — дашборд 03; просрочено — эскалация |
| `data_freshness` | Минуты с последнего факта (`last_callback_at` / `sent_at`) | green < 1 ч, yellow < 24 ч, иначе red | Проверить ELT-цикл, Airflow scheduler, доступ к Firebird |

Уровень здоровья клиники (`v_health_by_clinic_ui`, окно 24 ч): `critical` — доля ошибок ≥ 20% или очередь ≥ 100 документов; `warning` — доля ошибок ≥ 5% или очередь ≥ 20; иначе `ok`. Карточки `queue_24h` и `pending_backlog_24h` в таблице healthcheck на `02` скрыты; детализация очереди — на дашборде `03`.

Инцидент внеочередного наполнения журнала (строки с `LOGID` ниже watermark) обрабатывается задачей `reconcile_late_arrivals` в ELT; в BI отражается через актуализацию fact-слоя и сигнал `data_freshness`. Разбор отказов РЭМД и транспортных ошибок — дашборды `04` и `02` соответственно.

#### Компоновка `01_operational` (оперативный мониторинг)

Дашборд `01_operational.json` — контроль текущего потока документов и динамики по статусам. Источники: `v_rpt_documents_ui`, `v_rpt_error_category_breakdown_ui`, `v_rpt_semd_archive_ui` (график по дням). Период колбэков — `dwh_date_filter` по «Дата обработки» / «Обработано IPS». Все идентификаторы (`localUid`, `relatesToMessage`, `emdrId`, `LOGID`) и статус протянуты во все документные карточки. Healthcheck ELT и транспорт — дашборд `02`; очередь без ответа — `03`.

| Зона | Карточки | Источник | Зависимость от периода |
|---|---|---|---|
| Верх | «Последние операции» (таблица, до 50 строк); «Статусы за период» (пирог) | `v_rpt_documents_ui` | Да |
| Середина | «Ошибки по типу» (двойной пирог категория → тип); «Доля ошибок по клиникам» (топ-10 + «Остальные») | `v_rpt_error_category_breakdown_ui`, `v_rpt_documents_ui` | Да; пирог ошибок — только период / код СЭМД / JID (без статуса и идентификаторов) |
| Динамика | «Транзакции по дням и статусам» (stacked bar по календарным дням) | `v_rpt_semd_archive_ui` | Да; ось — «День (тренд)» |
| Ошибки | «Доля ошибок по СЭМД» (топ-10 типов); «% Ошибок за период» (таблица срезов с долей) | `v_rpt_documents_ui`, `v_rpt_error_interpretations_ui` | Да |

Универсум документов — `COUNT(DISTINCT «Документ (ключ учёта)»)` по всем 4 статусам; сумма сегментов пирога «Статусы за период» равна этому итогу. Универсум ошибок — `async_error` + `network_error`. Карточка «% Ошибок за период» показывает только срезы с хотя бы одной ошибкой; знаменатель доли — финализированные документы в срезе (клиника × тип СЭМД × тип ошибки).

#### Компоновка `02_service` (сервис интеграции)

| Зона | Карточки | Зависимость от периода |
|---|---|---|
| Верх | Почасовые тренды сетевых и async-ошибок; пирог «async vs network»; пирог уровней healthcheck | Тренды и пирог ошибок — да; healthcheck — снимок текущего состояния (в пироге те же сигналы, что в таблице, без очереди callback) |
...
Дашборд `06_semd_archive.json` — библиотека документов и поиск по идентификаторам. Единый источник — `v_rpt_semd_archive_ui` (обёртка над `v_rpt_documents_ui` с колонкой «СЭМД (архив)» для документов без кода НСИ). Период — `dwh_date_filter` по «Дата обработки». Все фильтры (код СЭМД, JID, `localUid`, `relatesToMessage`, `emdrId`, статус, `LOGID`) привязаны ко всем карточкам.

| Зона | Карточки | Зависимость от периода |
|---|---|---|
| Верх (KPI) | «Всего документов»; «Топ по типу СЭМД» (топ-8 + «Остальные»); «Топ по клинике» (топ-8 + «Остальные») | Да |
| Середина | «Всего клиник» (`COUNT(DISTINCT JID)`) | Да |
| Динамика | «Документы по дням» (bar по «День (тренд)») | Да; field filter `dwh_date` привязан к «День (тренд)» |
| Низ | «Архив СЭМД» (таблица, до 1000 строк, сортировка по дате обработки) | Да |

Счётчики и срезы используют один документный универсум: `COUNT(DISTINCT «Документ (ключ учёта)»)` по всем 4 статусам. Колонка «СЭМД (архив)» подставляет подпись для документов с пустым кодом НСИ или с ошибкой без определённого типа. `relatesToMessage` в колонке «Связанное сообщение» — коррелятор запрос–ответ в XML, не идентификатор документа.

### Финансовая модель (`05_executive.json`)

Управленческий дашборд интеграции с ЕГИСЗ. Grain: JID / документ / статус / канал; тело СЭМД не используется. Источник документных метрик — `v_rpt_documents_ui`. Иерархия блоков: NSM → Деньги → Здоровье сервиса → Выручка под риском.

| Метрика | Формула | Зависимость от фильтра «Период» |
|---|---|---|
| Успешных СЭМД (NSM) | `COUNT(DISTINCT документ)` WHERE `Статус (код) = success` | Да |
| Всего документов | `COUNT(DISTINCT документ)` по всем статусам, включая `waiting` | Да |
| First-pass acceptance (%) | `success` / все документы × 100, включая `waiting` | Да |
| MRR (trailing-30d), ₽ | `COUNT(DISTINCT JID` с документом за последние 30 дней) × 10 000 | Нет (trailing-30d от `CURRENT_DATE`) |
| ARR (trailing-30d), ₽ | MRR × 12 | Нет |
| Активных JID (trailing-30d) | `COUNT(DISTINCT JID)` с ≥ 1 документом за 30 дней | Нет |
| ₽ за успешный СЭМД | MRR (trailing-30d) / `COUNT(success)` за 30 дней | Нет |
| Динамика MRR/ARR/активных JID | Trailing-30d на каждую дату, окно последних 90 дней | Нет |
| ₽ под риском | `SUM(JID` с 0 успешных за период) × 10 000 | Да (`mgmt_period`) |
| ₽ под риском (затык) | `SUM(JID`, где `waiting` / `docs` > 80%) × 10 000 | Да |
| Сегменты ценности | По объёму документов за период: тяжёлые (≥ 1000), средние (50–999), спящие (< 50); MRR сегмента = JID × 10 000 | Да |

Тариф: фиксированная ставка 10 000 ₽ / JID / месяц. MRR и ARR считаются по trailing-30d (скользящие 30 дней от текущей даты), а не по календарному месяцу: при календарном месяце счётчик активных JID обнуляется 1-го числа. Помесячные MRR Waterfall, NRR и Logo retention появятся после накопления ≥ 2 закрытых календарных месяцев данных.
...
| Materialized view | Предрасчитанная витрина PostgreSQL для BI-запросов (`v_egisz_documents_daily_ui`). Обогащённая витрина `v_egisz_documents_enriched_ui` — persistent-таблица, сопровождаемая инкрементально в transform. |
