...
Для каждого элемента ответа сохраняется пара `code` / `message`. Человекочитаемая формулировка становится типом ошибки без перефразирования: из неё убираются только переменные реквизиты экземпляра. Технические сообщения (XPath, XML, Schematron, исключения и машинные ответы) и ответы без текста получают интерпретацию по правилам.
...
| `rpt_error_breakdown` | Элементы `documents.error_details` → уникальные пары документ × отображаемый тип с OID НСИ; категория и служебные признаки по `classification_type` |
