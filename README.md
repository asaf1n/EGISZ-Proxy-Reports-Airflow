...
Функции `public.egisz_error_interpretation_item`, `public.egisz_error_interpretation_row`, `public.egisz_error_messages_row` и `public.egisz_error_interpretation_type` интерпретируют и группируют ошибки для дашбордов. Справочные правила хранятся в `public.egisz_error_interpretation_rules`, а отдельная витрина `public.v_rpt_error_interpretations_ui` раскрывает ошибки построчно:
...
* `remd` / `рэмд` — в аналитике показывается как «Ошибка регистрации в РЭМД»; источник этого лейбла — ответы с `ns2status=error`;
* XSD/cvc — короткая подсказка по XML-валидации;
* ошибки по ФРМР, ГИП, НСИ, подписям и `getDocumentFile` схлопываются в человекочитаемые типы по реальным накопленным отказам, например «Должность врача не соответствует данным ФРМР», «Данные пациента не соответствуют ГИП», «Документ уже зарегистрирован в РЭМД», «Не удалось получить файл ЭМД из предоставляющей ИС»;
* без ошибок в UI-сводке возвращается пустая строка или «Успешно» для успешных документов.

Для BI-группировки используется отдельная функция `public.egisz_error_group_type(code, message)`. Она нужна для детализации причин внутри отказов РЭМД, при этом сетевые ошибки остаются отдельным общим блоком `Сетевая ошибка`.
...
* переменные идентификаторы и персональные значения схлопываются в плейсхолдеры (`<uuid>`, `<n>`, `<ФИО>`, `<поле>` и т.п.), чтобы не порождать отдельные типы на каждое значение;
* если справочного правила недостаточно, ошибка попадает в общий тип `Ошибка регистрации в РЭМД`.

Главная витрина `public.v_egisz_transactions_enriched_ui` отдаёт `Ошибки JSON` как человекочитаемый текст из XML `<ns2message>`. В аналитике общий тип `Ошибка регистрации в РЭМД` строится из статуса ответа `ns2status=error`, а детализация и текст для пользователя берутся из `ns2message` и подготовленных ELT-представлений/функций. Дублирующие служебные колонки с сырым JSON в пользовательскую витрину не выводятся.
