...
Для BI-группировки используется отдельная функция `public.egisz_error_group_type(code, message)`. Она нужна для детализации причин внутри ошибок асинхронного ответа РЭМД, при этом сетевые ошибки остаются отдельным общим блоком `Сетевая ошибка`.
