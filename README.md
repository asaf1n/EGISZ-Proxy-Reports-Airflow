...
Подпись типа дополняется справочником НСИ там, где класс отказа объявил шаблон (`dim_error_rules.nsi_dictionary_pattern`): без него отказы по разным справочникам неразличимы. Наименование берётся из `dim_nsi_dictionary` — краткая подпись, если она заведена, иначе официальное наименование НСИ 805. Справочник вне 805 остаётся в подписи голым OID.
