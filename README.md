...
На момент написания seed содержит **80 активных правил**, сводящихся к ~68 каноническим типам и **10 категориям** `error_category` (плюс `Прочие` для нераспознанного). Источник истины — `db/parts/30_error_rules.sql`; новое правило добавляется через `INSERT ... ON CONFLICT (rule_code) DO UPDATE`.
...
| Ошибки регистрации в РЭМД | Документ уже/не зарегистрирован, аннулирован, дубликат, метаописание, доступ. | `NOT_UNIQUE_PROVIDED_ID`, `DOCUMENT_NOT_FOUND`, `ATTRIBUTE_MISMATCH`, `ACCESS_DENIED`, `DUPLICATE_REQUEST` |
...
| 43 | Организация / ИС | Организация не найдена в реестре РЭМД | `ORGANIZATION_NOT_FOUND` | — |
| 44 | Организация / ИС | Организация не зарегистрирована в РЭМД | `ORGANIZATION_NOT_REGISTERED` | — |
| 45 | Организация / ИС | Лицензия организации не найдена | `ORGANIZATION_LICENSE_NOT_FOUND` | — |
| 46 | Организация / ИС | Несоответствие данных организации в ФРМО | — | `(ОГРН\|ОКПО\|КПП\|ИНН)...(СЭМД\|ФРМО)...не совпадает` |
| 47 | Организация / ИС | ИС зарегистрирована в РЭМД, но не активна | `DISABLED_RMIS` | — |
| 48 | Организация / ИС | ИС не зарегистрирована в РЭМД | `NO_RMIS` | — |
| 49 | Организация / ИС | Ошибки организации (generic fallback) | — | `организаци\|ОГРН\|ФРМО\|лицензи` |
| 50 | Получение файла ЭМД | Сервис предоставляющей ИС недоступен | `MIS_NOT_AVAILABLE` | — |
| 51 | Получение файла ЭМД | Запись ЭМД не найдена в предоставляющей ИС | `REGISTRY_ITEM_NOT_FOUND` | — |
| 52 | Получение файла ЭМД | ИС не передала файл ЭМД в ответе getDocumentFile | `FILE_WAS_NOT_SENT` | — |
| 53 | Получение файла ЭМД | Не удалось получить файл ЭМД из предоставляющей ИС | `RMIS_ERROR` / `GET_DOCUMENT_FILE_ERROR` | код / текст `getDocumentFile` |
| 54 | ЭП и сертификаты | Сертификат ЭП истёк | — | `сертификат...истёк\|certificate...expired` |
| 55 | ЭП и сертификаты | Сертификат ЭП отозван | — | `сертификат...отозван\|certificate...revoked` |
| 56 | ЭП и сертификаты | Недействительный сертификат подписи | — | `CANT_BUILD_CERT_CHAIN\|цепочк...сертификат` |
| 57 | ЭП и сертификаты | Сертификат недействителен на дату создания документа | — | `DOC_DATE_MISMATCH_CERT_NOT_BEFORE` |
| 58 | ЭП и сертификаты | Срок действия сертификата организации истёк | `CANT_BUILD_CERT_CHAIN_TO_ACCREDITED_CA_CERT` | — |
| 59 | ЭП и сертификаты | Не удалось проверить электронную подпись | `SIGNATURE_VERIFICATION_ERROR` | — |
| 60 | ЭП и сертификаты | Недоступен сервис проверки статуса сертификата (CRL/OCSP) | — | `CRL\|OCSP\|сервис...проверк...сертификат` |
| 61 | ЭП и сертификаты | Данные подписи не соответствуют данным документа | `VALUE_MISMATCH_METADATA_AND_CERTIFICATE` | — |
| 62 | ЭП и сертификаты | Подпись роли не соответствует требованиям РЭМД | `ROLE_OCCURRENCE_MISMATCH` | — |
| 63 | Технические ошибки РЭМД | РЭМД не смог обработать запрос | `RUNTIME_ERROR` | `Невозможно обработать запрос` |
| 64 | Технические ошибки РЭМД | Техническая ошибка на стороне РЭМД | `INTERNAL_ERROR` / `RUNTIME_ERROR` | код / текст `внутренн...ошиб` |
| 65 | Технические ошибки РЭМД | Таймаут асинхронной обработки на стороне РЭМД | `ASYNC_RESPONSE_TIMEOUT` | — |
| 66 | Технические ошибки РЭМД | Недоступен сервис проверки подписи (УЦ) на стороне РЭМД | `CA_UNAVAILABLE` / `CA_INACCESSIBILITY` | — |
| 67 | Ошибки связи | Сетевая ошибка | — | `network\|connection\|timeout\|таймаут\|соединени` |
| 68 | Прочие | Неизвестная ошибка | — | финальный fallback (ни одно правило не сработало) |
