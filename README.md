...
| Недельная динамика | SLI по неделям, контрольная карта XmR, структура ошибок по категориям | `rpt_documents_weekly`, `rpt_error_breakdown_weekly` |
...
Для каждого элемента ответа сохраняется пара `code` / `message`. Тип ошибки — наименование класса по правилам. Текст ответа в тип не переносится: в нём остаются реквизиты экземпляра (ФИО и e-mail из сертификата, даты рождения, фамилии пациентов), а тип уходит в фильтры и сводки, в том числе клиентские. Текст хранится в `message` и `error_text`.

Классификация для категории и зоны ответственности выполняется отдельно от отображаемой формулировки. Каждый атом отказа проходит четыре приоритета по порядку; первый с совпадением завершает поиск, последующие приоритеты не вычисляются.
...
flowchart TD
    ...
    FB --> SPLIT["documents.error_details — отдельные элементы<br/>rpt_error_breakdown — одна строка на тип"]
```
...
| Поле | Содержание |
| ---- | ---------- |
| `error_types` | Полный список отображаемых типов через `·`, для журнала документов и переходов из отчётов; это не источник границ отдельных ошибок |
| `error_details` | JSON-массив отдельных ошибок: исходные `code` и `message`, `error_type` (наименование класса), ключ классификации `classification_type`, OID НСИ конкретного элемента |
| `error_text` | Исходный текст из `<message>` **без редактирования** |

`rpt_error_breakdown` строится из сохранённых `documents.error_details`: одна строка соответствует отдельному типу ошибки документа. Повтор одного типа не увеличивает число документов. Категория, зона ответственности и повторяемость берутся из `dim_error_type_group` по `classification_type`. Поле `base_error_type` содержит тип без подписи НСИ и используется для перехода в журнал документов. До переноса старого архива строки с `error_details IS NULL` читаются из прежнего списка типов; OID НСИ для них извлекаются из сохранённого текста по шаблону класса.

Подпись НСИ имеет формат `<тип> (справочник: <полное наименование>, OID <идентификатор>)`, наименование — из `dim_nsi_dictionary.name`. Скобки вместо ` · ` нужны, чтобы подпись не читалась как несколько типов из `documents.error_types`. OID извлекается шаблоном класса (`dim_error_rules.nsi_dictionary_pattern`) из конкретного элемента ответа. Ошибки по двум справочникам одного типа дают две строки; общей подписи «несколько справочников» нет. Если справочника нет в локальном реестре НСИ, показывается его OID без выдуманного наименования.

Фильтр «Тип ошибки» привязан к `rpt_error_breakdown.error_type` во всех карточках анализа ошибок. Несколько выбранных значений означают «любой из выбранных типов». Отбор документа выполняется через `EXISTS` или подсчёт уникальных `dwh_id`; другие ошибки того же документа не исключают его из результата. Знаменатели долей сохраняют исходный набор документов периода.

Перенос существующего архива выполняется после применения схемы командой `psql -X -U egisz -d dwh_egisz -v ON_ERROR_STOP=1 -f scripts/backfill_error_details.sql`. Скрипт обрабатывает выбранный ответ каждого документа пакетами, не меняет курсоры ELT и повторно не переносит заполненные строки. После переноса он выводит число документов, для которых не удалось восстановить элементы, обновляет витрины и статистику. Анализ причин и условия выкладки описаны в [аудите типов ошибок](docs/error-types-audit.md).
...
| Наименования справочников | `dim_nsi_dictionary` | Снимок НСИ `1.2.643.5.1.13.13.99.2.805` вер. 6.19 (465 записей): OID справочника → наименование дословно + краткая подпись `short_name`; подписывает предмет отказа в `rpt_error_breakdown.error_type`. Справочники вне 805 показываются без расшифровки |
...
| `rpt_error_breakdown` | Элементы `documents.error_details` → уникальные пары документ × тип с OID НСИ; категория и служебные признаки по `classification_type` |
...
| `05_executive.json` | Управленческий дашборд | Руководство | «Обзор» (ценность и качество, здоровье сервиса, пульс за скользящие 7 дней, ориентировочные деньги, выручка под риском по `rpt_documents`), «Динамика по неделям» и «Динамика по месяцам» (состав исходов, контрольная карта XmR по неделям, структура ошибок по `rpt_*_weekly` / `rpt_*_monthly`) |
...
Аудитория — руководство. Два фильтра шапки действуют на все три вкладки; часть карточек «Обзора» намеренно не реагирует на «Период» (см. ниже). Текстовых подписей у рядов нет: знаменатель и окно названы в имени карточки («% от ответов», «за последние 7 дней»), подробности — в её описании; единственная текстовая оговорка — о плоской ставке рублёвых карточек.
...
| ------- | --- | ------------ | ------------------- |
| Обзор | Ценность и качество | «Успешных СЭМД за период», объём потока, «Доля успеха, %», «С первой подачи, % от ответов» | Да |
| Обзор | Здоровье сервиса | «С первой подачи, % от успешных», «Отказов РЭМД, %», «Ошибок связи, %», «Ответ не получен, %» | Да |
| Обзор | Пульс | Успешные СЭМД, доля успеха и активные клиники «за последние 7 дней» на каждый день (14 точек) | Нет |
| Обзор | Деньги | MRR / ARR / активные JID за последние 30 дней, стоимость успешного СЭМД, MRR и активные JID по дням за 90 дней | Нет |
| Обзор | Выручка под риском | Сегменты клиник по объёму; «Клиник без успехов (от 10 документов)», их MRR и очередь оттока | Да |
| Обзор | Выручка под риском | «Замолчавших клиник (нет документов 7 дней)», их MRR и очередь | Нет |
| Динамика по неделям / месяцам | SLI во времени | Состав исходов, объём, контрольная карта XmR по неделям, структура ошибок по категориям — вся история | Нет |

**Пульс — скользящее окно, а не календарный период.** Каждая точка — итог за предыдущие семь суток, окно шагает по дням. Недельное окно снимает разницу буден и выходных, поэтому «изменение» под числом читается как сдвиг недельного итога, а не как колебание одного дня. Свежая точка всегда полная — в отличие от текущей календарной недели. Слово «последние» в имени означает окно, привязанное к текущему дню: фильтр «Период» на такие карточки не действует.

**Первая подача.** «С первой подачи, % от ответов» — успешные с `attempt_count ≤ 1` от документов **с ответом РЭМД**, то есть на том же знаменателе, что «Доля успеха, %». Документ относится к периоду по дате итога (`ips_date`), а не по дате первой подачи. Разрыв между плитками — успехи после пересдачи: документы, дошедшие только со второй и последующих попыток. Плитка-спутник «С первой подачи, % от успешных» показывает ту же величину от числа успешных: её дополнение до 100 % — доля успехов, потребовавших пересдачи.

**Динамика — вся история.** Фильтр «Обработано IPS» к вкладкам динамики не подключён: периодические витрины и так короткие, а контрольная карта требует всего ряда закрытых недель. Действует только срез по клинике. Графики «Динамика MRR» и «Динамика активных JID» начинаются через 30 дней после первого дня истории: раньше тридцатидневное окно ещё заполняется, и подъём читался бы как рост базы.
...
**Выручка под риском.** При плоской ставке выручка распределена по JID равномерно, а ценность — нет. Два рабочих списка Customer Success, а не бухгалтерская оценка потерь. «Ноль ценности» — JID от 10 документов за период и без единого успешного СЭМД; порог объёма отсекает единичные попытки (проба подключения — не риск оттока), в очереди рядом с исходами стоят ИНН и основной тип ошибки (самый частый элемент `error_types` по документам JID за период). «Замолчавшие» — JID с документами за последние 30 дней и без документов за последние 7 суток: ещё в активной базе под MRR, но поток остановился; окна привязаны к текущему дню, поэтому фильтр «Период» на этот список не действует. Прежняя величина «затык доставки» (свыше 80 % документов в обработке) снята: за всю историю она давала 0 ₽.

**Чего на дашборде нет и почему.** NRR, GRR, logo retention и MRR-waterfall требуют не менее двух закрытых календарных месяцев истории по одной и той же базе; до накопления такой истории карточки-заглушки на дашборд не выводятся. Ранний признак отвала до этого момента даёт «Активных клиник за последние 7 дней» — недельное окно проседает раньше тридцатидневного.
...
| Один знаменатель на семейство долей | Корпус документов с ответом РЭМД зафиксирован для всех долей качества и совпадает с остальными отчётами; знаменатель назван в имени карточки и её описании |
...
| Отделение шума от сигнала | Контрольная карта XmR по закрытым неделям: центр — средняя доля ошибок, границы — средняя ± 2,66 × средний скользящий размах. p-карта не подходит: при 55–75 тыс. документов в неделю биномиальная σ = 0,18 п.п. против фактического разброса 3,86 п.п., и почти каждая точка выпадала за p̄ ± 3σ. Месячная карта появится после 12 закрытых месяцев |
...
| НСИ «Реестр справочников, использующихся в руководствах по реализации структурированных электронных медицинских документов» (OID `1.2.643.5.1.13.13.99.2.805`, версия 6.19) | Справочники НСИ, предписанные руководством (`dim_nsi_semd_guide_dictionary`); наименования справочников для подписи предмета отказа (`dim_nsi_dictionary`) |
