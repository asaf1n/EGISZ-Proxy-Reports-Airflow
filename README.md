...
Обмен с РЭМД асинхронный. МИС формирует CDA-документ, подписывает его, отправляет через интеграционный шлюз и получает технический ответ о приёме. Затем приходит callback с итогом: документ зарегистрирован и получил `emdrId`, находится в обработке или отклонён с ошибками.
...
| `70_views_core.sql` | `v_egisz_transactions_enriched_ui`, `v_rpt_error_interpretations_ui`. |
...
| Статус | Значение |
|---|---|
| `success` | Документ зарегистрирован в РЭМД. |
| `error` | РЭМД вернул финальный отказ или зафиксирована сетевая ошибка. |
| `pending` | Документ принят или находится в обработке. |
| `unknown` | Callback получен, но статус не распознан правилами. |

`error_type` заполняется только для `status = 'error'`. Для `pending` и `unknown` значение остаётся `NULL`.
...
|---|---|---|---|
| `01_operational.json` | Операционный поток документов, статусы, ошибки, динамика. | `v_egisz_transactions_enriched_ui`, `v_rpt_error_category_breakdown_ui` | Период, код СЭМД, JID, идентификаторы |
...
| `05_executive.json` | Управленческие KPI: активные JID, объёмы, статусы, MRR/ARR по фикс-тарифу. | `v_egisz_transactions_enriched_ui`, `v_rpt_documents_no_response_ui` | Период |
...
```sql
REFRESH MATERIALIZED VIEW CONCURRENTLY public.v_egisz_transactions_enriched_ui;
