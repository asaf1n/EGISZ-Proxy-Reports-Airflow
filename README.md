...
| Визуализация | Metabase v0.62.1 | 8 дашбордов поверх Metabase Models и `v_rpt_*_ui` (гибрид Query Builder + native SQL) |
...
Восемь дашбордов закрывают разные роли — от дежурной смены до клиентской BI. Карточки читают готовые представления DWH (`v_egisz_documents_enriched_ui`, `v_rpt_*_ui`) через **Metabase Models** (`metabase_models/`) и Query Builder; сложная логика (MRR, window, join) остаётся в native SQL. Фильтры дашборда и drill-through в **Архив СЭМД** (`06`) работают без деплоя JSON.
...
Дашборды — код в `metabase_dashboards/`, модели в `metabase_models/`, импорт при старте через `metabase/setup-dashboards.sh` (синхронизация моделей — `metabase/sync-models.sh`). Фильтры по клинике (`JID`), типу СЭМД и статусу — в `field_filter_defaults.yaml` для native-карточек и `metabase-parameter-targets` для Query Builder. В UI аналитиков — поле **«Статус»** (4 значения); машинный **«Статус (код)»** скрыт в моделях и используется только в DWH.
...
metabase/setup-dashboards.sh       — импорт дашбордов и моделей при старте контейнера
metabase/sync-models.sh            — provisioning Metabase Models + semantic types
metabase_models/*.json             — 6 моделей витрин (Документы, Разбивка ошибок, Архив, …)
