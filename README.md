...
|---|---|---|---|---|
| `01_operational.json` | Операционный поток документов, статусы, ошибки, динамика. | 9 | `v_rpt_documents_ui`, `v_rpt_error_category_breakdown_ui` | Период, код СЭМД, JID, идентификаторы |
| `02_service.json` | Healthcheck ETL, канала и прокси-БД. | 15 | `v_health_*_ui`, `v_rpt_documents_ui`, `v_stg_channel_errors_by_document`, `v_rpt_network_errors_detail_ui` | Период, код ошибки, JID |
...
| `04_quality_and_errors.json` | Качество данных и детализация отказов РЭМД. | 22 | `v_rpt_documents_ui`, `v_rpt_error_category_breakdown_ui`, `v_rpt_network_errors_detail_ui` | Период, категория, тип ошибки |
